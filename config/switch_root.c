...
static int last_char_is(const char *s, char c)
{
    int len = strlen(s);
    if (len && s[len-1] == c) {
        return 1;
...
static char* concat_path_file(const char *path, const char *filename)
{
    ...
    sprintf(outbuf, "%s%s%s", path, (lc ? "/" : ""), filename);
    return outbuf;
}
...
int switch_root_main(char *newroot, char *newinit)
{
	...
	if (stat("/init", &st) != 0 || !S_ISREG(st.st_mode)) {
		printf("/init is not a regular file\n");
        return 1;
	}
	...
	if ((unsigned)stfs.f_type != RAMFS_MAGIC
	 && (unsigned)stfs.f_type != TMPFS_MAGIC
		...
		printf("root filesystem is not ramfs/tmpfs\n");
        return 1;
	}
